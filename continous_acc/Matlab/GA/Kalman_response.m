...
for j=1:c
    ...
    [q, ~, ~] = Gyro_lib_quat(P, bw*((pi/180)*1/fs), gyro_data*((pi/180)*1/fs), X(5,j), X(6,j),...
    acc_data, mag_data, fn, mn, 1/fs, R);  
        
    %correcting the bad scalar matrices 
    q(isnan(q(:,1)),1:4) = 0.5; %replace NaN by unit vector
    %q(isnan(q(:,2)),2:4) = 0;
    if(norm(q(9,:)) == 0) % if this position is all zero
        q(9,:) = [1 0 0 0];
    end
    
    q = quatconj(q);    %the gyrolib returns the conjugate of the quaternion!!!!

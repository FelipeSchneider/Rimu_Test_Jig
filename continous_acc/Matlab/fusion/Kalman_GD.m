function [ qUpdate ] = Kalman_GD(Acc, GyroRate, Magn, fs, gyro_off, gyro_var,...
                            sigmaR, P_guess, acc_lpc, mag_lpc, quat_init)
%[ quaternion ] = Kalman_GD(Acc, GyroRate, Magn, fs, gyro_off, gyro_var,...
%                            sigmaR, P_guess, acc_lpc, mag_lpc, quat_init)
%Kalman_GD make the fusion of data based on the Gradient Descent version of the filter
...
%     GyroRate(3,i)=(GyroRate(3,i)+GyroRate(3,i-1))/2;
    ...
    qUpdate(:,i)=qPredicted(:,i)+K*(qOsserv(:,i)-H*qPredicted(:,i)); %ESSE TIPO DE OPERAÇÃO NÃO É DETERMINADA EM QUATERNIOS -> 
                        %É UMA IMPLEMENTAÇÃO DO KALMAN DIRETAMENTE EM QUATERNIOS, SEM LEVAR EM CONSIDERAÇÃO AS REGRAS BÁSICAS DE OPERAÇÕES EM QUATERNIOS

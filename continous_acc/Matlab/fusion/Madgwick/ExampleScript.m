...
for t = 1:length(time)
    AHRS.Update(giro_imu_dps(:,t)' * (pi/180), acc_imu_g(:,t)', mag_bno_gaus(:,t)');	% gyroscope units must be radians
...
[euler(:,1) euler(:,2) euler(:,3)] = quat2angle(quaternion);
euler = euler*180/pi;
[euler_bno(:,1) euler_bno(:,2) euler_bno(:,3)] = quat2angle(quaternios_bno');
euler_bno = euler_bno*180/pi;
...
figure(4);hold all;
subplot(311); plot(time,[euler(:,1) euler_bno(:,1)]'); grid on;
title('Angulos de Euler'); hold all; legend('BNO','IMU');ylabel('Yaw');
subplot(312); plot(time,[euler(:,2) euler_bno(:,2)]'); grid on;
hold all; legend('BNO','IMU');ylabel('Pitch');
subplot(313); plot(time,[euler(:,3) euler_bno(:,3)]'); grid on;

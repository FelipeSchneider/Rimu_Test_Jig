...
top_angle = [3000 90 90 90 90 90 90 120 150 3000 800 5000 20 10 10 30];
top_speed = [0 35 60 120 -120 -60 -20 120 300 0 -720 360 12 -30 50 -30];

base_angle = [3000 150 90 90 90 90 90 120 120 30 ];
base_speed = [0 350 120 180 -180 -120 -15 50 -45 15];

COM_jig_num = 11;

%rimu definitions
BLUETOOTH = 1;          %to use bluetooth set this flag
time_sample = 50;       %number of seconds that we will collect
...
figure(3);
subplot(311);plot(t_imu,[giro_imu_dps(1,:); giro_bno_dps(1,:)]'); grid on;
title('Giroscópio');ylabel('X [dps]');legend('Minimu','BNO','Location','northwest','Orientation','horizontal')
subplot(312);plot(t_imu,[giro_imu_dps(2,:); giro_bno_dps(2,:)]'); ylabel('Y [dps]'); grid on;
subplot(313);plot(t_imu,[giro_imu_dps(3,:); giro_bno_dps(3,:)]'); ylabel('Z [dps]'); grid on;
xlabel('Tempo [s]')

figure(4);
subplot(311);plot(t_imu,[acc_imu_g(1,:); acc_bno_g(1,:)]'); grid on;
title('Acelerômetro');ylabel('X [g]');legend('Minimu','BNO','Location','northwest','Orientation','horizontal')
subplot(312);plot(t_imu,[acc_imu_g(2,:); acc_bno_g(2,:)]'); ylabel('Y [g]'); grid on;
subplot(313);plot(t_imu,[acc_imu_g(3,:); acc_bno_g(3,:)]'); ylabel('Z [g]'); grid on;

figure(5);
subplot(311);plot(t_imu,[mag_imu_gaus(1,:); mag_bno_gaus(1,:)]'); grid on; grid on;
title('Magnetômetro');ylabel('X');legend('Minimu','BNO','Location','northwest','Orientation','horizontal')
subplot(312);plot(t_imu,[mag_imu_gaus(2,:); mag_bno_gaus(2,:)]'); ylabel('Y'); grid on;
subplot(313);plot(t_imu,[mag_imu_gaus(3,:); mag_bno_gaus(3,:)]'); ylabel('Z'); grid on;
xlabel('Tempo [s]')

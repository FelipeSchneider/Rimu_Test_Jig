...
for i=1:length(t_rec)/2
    ...
    ang(i) = mean(e_jig(index-200:index-100,3)); %yaw correction
...
[pitch_pol_yaw,pitch_S_yaw] = fit(ang',error_mean(:,1),'poly4');
[pitch_pol_roll,pitch_S_roll] = fit(ang',error_mean(:,2),'poly4');
[pitch_pol_pitch,pitch_S_pitch] = fit(ang',error_mean(:,3),'poly4');
pitch_S_yaw
pitch_S_roll
pitch_S_pitch
...
hold on; plot(ang, error_mean(:,1),'.','MarkerSize',20); hold on; plot(pitch_pol_yaw, 'predobs');
...
hold on; plot(ang, error_mean(:,2),'.','MarkerSize',20); hold on; plot(pitch_pol_roll, 'predobs');
grid on;
subplot(313);
...
hold on; plot(ang, error_mean(:,3),'.','MarkerSize',20); hold on; plot(pitch_pol_pitch, 'predobs');
grid on;
